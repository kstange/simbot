...
#
# Copyright (C) 2004, Kevin M Stange <kevin@simguy.net>
...
sub process_nlp {
	...
	&SimBot::debug(4, "nlp: full string: $request\n");
	...
	foreach my $text (@requests) {
		...
		&SimBot::debug(4, "nlp: this segment: $text\n");
		...
		foreach my $plugin (keys (%{SimBot::hash_plugin_nlp_verbs})) {
			&SimBot::debug(4, "nlp: current plugin: $plugin\n");
			foreach my $verb (@{${SimBot::hash_plugin_nlp_verbs}{$plugin}}) {
				&SimBot::debug(4, "nlp: current verb: $verb\n");
				my $verbmatch = $verb . "|"
					. $verb . "s|"
					. $verb . "ed|"
					. $verb . "ing";
		...
		&SimBot::debug(4, "nlp: verbs passed for: " . join(" ", @matches));
		...
		&SimBot::debug(4, "nlp: query type: $type\n");
		...
		@still_matches = ();
		&SimBot::debug(4, "nlp: queries passed for: " . join(" ", @matches));
		...
		@matches = @still_matches;
		@still_matches = ();
		&SimBot::debug(4, "nlp: subjects passed for: " . join(" ", @matches));
