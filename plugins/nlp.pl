	...
	foreach my $text (@requests) {
		...
		foreach my $plugin (@matches) {
			...
			foreach my $format (@{${SimBot::hash_plugin_nlp_formats}{$plugin}}) {
				...
				$format =~ s/\{q\}/(\\d+\\.?\\d*|couple|couple of|handful of|all|every|all the|no|some|few|an|the|many|several|plenty of|lot of|lots of|copious|excessive|dozen|dozens of|$verbmatch)/g;
