...
use constant CANT_FORGET => ('$nick: I don\'t know anything about $key.');
...
use constant BUT_X_IS_Y => ( '$nick: I thought $key $isare $factoid.', );
...
sub handle_chat {
    ...
    if ( $being_addressed && $content =~ m{^forget ([\'\-\w\s]+)}i ) {    #'
        ...
        if ( $info{$key} ) {
            ...
            &SimBot::send_message( $channel,
                &parse_message( &SimBot::pick(CANT_FORGET), $nick, $key ) );
        }
        ...
        && $content =~ m/where can (I|one) find ([\'\w\s]+)/i )
    {
        ...
        if ($also) {
            ...
            if ($being_addressed) {
                ...
                } ## end else [ if ( $info{$key} =~ m/\|\|/)
            } ## end if ($being_addressed)
        } else {
