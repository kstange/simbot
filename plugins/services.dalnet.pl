...
package SimBot::plugin::services::dalnet;
...
sub services_login {
    ...
    if ($pass && $user) {
                ...
                &SimBot::send_message("NickServ\@services.dal.net", "identify $pass");
    }
}
...
sub request_invite {
    ...
    if (&SimBot::option('services', 'pass')) {
                ...
                if($services_online && $logged_in) {
                        ...
                        &SimBot::send_message("ChanServ\@services.dal.net", "invite $channel");
...
sub request_voice {
    ...
    if (&SimBot::option('services', 'pass')) {
                ...
                if($services_online && $logged_in) {
                        ...
                        &SimBot::send_message("ChanServ\@services.dal.net", "voice $channel");
                        $shut_up = 0;
                } elsif ($services_online) {
                        &services_login($kernel);
                }
...
sub kick_user {
    my (undef, $channel, $user, $message) = @_;
        ...
        &SimBot::send_message("ChanServ\@services.dal.net", "kick $channel $user $message");
}
...
sub ban_user {
    my (undef, $channel, $user, $time, $message) = @_;
        ...
        &SimBot::send_message("ChanServ\@services.dal.net", "ban $channel " . &SimBot::hostmask($user) . "$message");
}
...
sub unban_user {
    my (undef, $channel, $user, $message) = @_;
        &SimBot::debug(3, "Asking Channel Service to unban $user (" .
                                   &SimBot::hostmask($user) .
                                   ") from $channel...\n");
        &SimBot::send_message("ChanServ\@services.dal.net", "unban $channel " . &SimBot::hostmask($user));
}
